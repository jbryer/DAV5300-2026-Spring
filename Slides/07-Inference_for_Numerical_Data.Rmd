...
# omp_date <- format(as.Date(rmarkdown::metadata$date, format = '%B %d, %Y') - 21, format = '%B %d')
# omp <- omp %>% dplyr::filter(`Class Date` == omp_date)
omp <- omp %>% dplyr::filter(`Class Date` == 'Inference for Categorical Data')
```
...
You each received a small pack of M&Ms. We want to know if each color of M&M is equally likely to be available. With your pack:

1. Count each color.

2. Scan the QR code and enter your results

]
.pull-right[
```{r, echo=FALSE, fig.height=4, fig.width=4}
qrcode::qr_code('https://forms.gle/qbfqT6S19DnePJmW6') |> plot(col = c('#FAFAFA', 'black'))
```
]

.center[
$H_0: \mu_{orange} = \mu_{red} = \mu_{green} = \mu_{yellow} = \mu_{blue} = \mu_{brown}$
...
results <- results |> dplyr::filter(Timestamp >= as.POSIXct('2026-03-10 8:00:00'))
results_melt <- results |>	reshape2::melt(id.vars = 'Timestamp') |> dplyr::rename(color = variable)
tab <- psych::describeBy(results_melt$value, group = results_melt$color, mat = TRUE, skew = FALSE) |>
	dplyr::rename(color = group1)
tab
```
