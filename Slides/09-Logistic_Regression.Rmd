...
.pull-left[
Please see [spring2026.dav5300.net/assignments/project.html](https://spring2026.dav5300.net/assignments/project.html) for details on the project proposal.

Please sign-up for a time to meet with me (via Zoom) on April 14th to discuss your project. Please have your proposal ready (template on the course website) and come prepared with questions. 


]
.pull-right[
```{r, echo=FALSE, fig.width=5, fig.height=5}
signup_link <- 'https://docs.google.com/spreadsheets/d/1Xf2fqKove8y22B-5d_bh662lK4R2jmxIcJer9L81LKo/edit?usp=sharing'
qrcode::qr_code(signup_link) |> plot(col = c('#FAFAFA', 'black'))
```
[Scan the QR code or click here to sign-up](`r signup_link`)
]
...
omp <- omp %>% dplyr::filter(`Class Date` == 'Linear Regression')
```
...
loglikelihood <- function(parameters, predictor, outcome) {
	...
	ll.vec <- dnorm(outcome, mean = a + b * predictor, sd = sigma, log = TRUE)
	return(sum(ll.vec))
}
```
